...
public class ProgramManagementTest extends AbstractProgramLevelTest {
    ...
    public void updatePatientStatusPatientProfile_MHM_T120(){
        ...
        patientNew.setFirstName("PatientUpdatedFirstName" + getRandomString());
        patientNew.setLastName("PatientUpdatedLastName" + getRandomString());
        patientNew.setEmail(getRandomString()+"@gmail.com");
        patientNew.setZipCode("54321");
        patientNew.setCellPhone(getGeneratedPhoneNumber());
        patientNew.setTimezone("Eastern Time (New York)");

        site.programSteps().addNewProgram(clientName, programName, ProgramAccess.PUBLIC);
        site.programSteps().addNewPatientAllFields(patient, client, programName);

        Patient updatedPatient = site.programSteps().updatePatientAllFields(patientNew, client, programName);
        Patient actualPatient = site.programSteps().getPatientObjectByName(patientNew, client, programName);

        boolean bol = updatedPatient.equals(actualPatient);

        Assert.assertEquals(updatedPatient, actualPatient, "Patient  details were are not updated");
